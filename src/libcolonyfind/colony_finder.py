...
class ColonyFinder:
    def __init__(
        ...
        """path to images of petri dishes. used for running OpenCFU and image annotation"""
        self.csv_out_path = csv_out_path
        """path to csv files generated by OpenCFU. used for parsing colony coordinates"""

        self.raw_coords = {}
        """Holds coords of colonies as generated by OpenCFU. This is set by parse_cfu_csv"""
        self.valid_coords = {}
        """Holds coords of colonies that are valid for sampling. This is set by remove_invalid_colonies"""
        self.final_coords = {}
        """Holds up to 96 coords of colonies that are valid for sampling. This is set by remove_extra_colonies"""
    ...
    def run_full_proc(self):
        """Runs the full colony finding process. This includes running OpenCFU, parsing the resultant csvs, removing invalid colonies, and removing extra colonies. Does not annotate."""
        self.run_cfu()
    ...
    def parse_cfu_csv(self):
        """
        Parses csv files generated by OpenCFU. Process control saves these files in the output folder. This happens
    ...
    def remove_invalid_colonies(self):
        ...
        - Too close to the x-axis origin to be sampled (if in first two petri dishes)
    ...
    def annotate_images(self):
        """
        Acts upon the images in the 'raw_image_path' folder.

        Annotates images by:
        - Writes the well the colony is destined for next to each colony
        - Draws a circle of the same radius as CONSTANTS.MIN_COLONY_RADIUS in the center of the colony. This represents the mechanical and optical error introduced to the coordinate.
        ...
        **Returns** a dict of annotated images, with the image name as the key, and the annotated image as the value
        ...
        try:
            ...
            for image_name, coord_list in coords.items():
                ...
                # Draw circle around the center of the petri dish. 
                # Colonies outside of this circle are not valid for sampling
                cv2.circle(
                    image,
