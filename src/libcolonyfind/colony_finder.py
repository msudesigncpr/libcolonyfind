...
class ColonyFinder:
    def __init__(self, raw_image_path, csv_out_path, image_names, annotated_image_output_path = None): 
    ...
    def run_full_proc(self):
        ...
        if self.annotated_image_output_path is not None: self.annotate_images(coords, self.raw_image_path, self.annotated_image_output_path)
        baseplate_coords = self.generate_baseplate_coords(coords)
        return baseplate_coords

    def run_cfu(self, raw_image_path, csv_out_path, cfu_win_path = CONSTANTS.CFU_WIN_PATH):
        """
        ...
        try: 
            for image in os.listdir(images_for_cfu_win_path):
                ...
                cfu_image_wsl_path = images_for_cfu_wsl_path + '/' + base_image_name + ".jpg" # where cfu will look for img
                cfu_coord_wsl_path = cfu_csv_wsl_dump_path + '/' + base_image_name + '.csv' # where cfu will place coords to colonies it finds (ex /mnt/c/Users/colon/Downloads/GUI/src/cfu_coords/dish_0.csv)
    ...
    def remove_invalid_colonies(self, coords, petri_dish_roi = CONSTANTS.PETRI_DISH_ROI, min_colony_dist = CONSTANTS.MIN_COLONY_DISTANCE, min_colony_radius = CONSTANTS.MIN_COLONY_RADIUS, img_height = CONSTANTS.IMG_HEIGHT, img_width = CONSTANTS.IMG_WIDTH):
        '''
        ...
        for image_name, coord_list in coords.items():
            ...
            for main_colony_coords in coord_list:
                ...
                if self.distance_from_center(main_colony_x, main_colony_y) > petri_dish_roi: 
                    bad_colony = True
                    ...
                    for neighbor_colony_coords in coord_list: # mmm O(n^2). great.
                        ...
                        main_is_doublet = self.distance_between_colonies(main_colony_x, main_colony_y, main_colony_r, neighbor_colony_x, neighbor_colony_y, neighbor_colony_r) < min_colony_dist 

                        main_is_out_bounds = (self.baseplate_coord_transform(main_colony_x, main_colony_y)[1] < -27.77) and (petri_dish_counter == 0 or petri_dish_counter == 1) # TODO: Make petri dish counter numbers constants
                        print("Colony is out of bounds: ",self.baseplate_coord_transform(main_colony_x, main_colony_y)[1] > 27.77)
            ...
            logging.info("DETECTED:...........%s | BAD:............ %s | GOOD:....................%s", total_colonies_in_image, bad_colony_counter,  len(temp_coords[image_name]))
    ...
    def baseplate_coord_transform(self, x, y, gsd_x = CONSTANTS.GSD_X, gsd_y = CONSTANTS.GSD_Y, img_width = CONSTANTS.IMG_WIDTH, img_height = CONSTANTS.IMG_HEIGHT):
        '''
    ...
    def remove_extra_colonies(self, coords):
        ...
        try:
            ...
            temp_dict = {image_name: [] for image_name in coords.keys()} # copy keys from coords to temp_dict but not the values
                ...
                while num_colonies_to_sample < 96:
                    for image_name, coord_list in coords.items():
                        if len(coord_list) > 0: # TODO: if the coord is in the two petri dishes next to the bar and they are beyond some limit, don't sample them
                            ...
                            if len(coord_list) == 0:
                                logging.info("All colonies in %s will be sampled from", image_name)
    ...
    def annotate_images(self, coords, annotation_image_input_path, annotation_output_path, wells = CONSTANTS.WELLS, image_height = CONSTANTS.IMG_HEIGHT, image_width = CONSTANTS.IMG_WIDTH, petri_dish_roi = CONSTANTS.PETRI_DISH_ROI, min_colony_radius = CONSTANTS.MIN_COLONY_RADIUS, gsd_x = CONSTANTS.GSD_X, gsd_y = CONSTANTS.GSD_Y):
        '''
        ...
        try:
            ...
            for image_name, coord_list in coords.items():
                ...
                image = cv2.imread(os.path.join(annotation_image_input_path, str(image_name) + '.jpg'))
        ...
        try:
            ...
            for _, coord_list in coords.items():
                    for colony_coord in coord_list:
                        ...
                        colony_coord = self.baseplate_coord_transform(colony_coord[0], colony_coord[1])
